...
async function defenderDeploy(hre, factory, opts, ...args) {
    const client = (0, utils_1.getDeployClient)(hre);
    ...
    if (deploymentResponse.address === undefined) {
        throw new upgrades_core_1.UpgradesError(`Deployment response with id ${deploymentResponse.deploymentId} does not include a contract address`, () => 'The Hardhat Upgrades plugin is not currently compatible with this type of deployment. Use a relayer for your default deploy approval process in Defender.');
    }
    if (deploymentResponse.txHash === undefined) {
        throw new upgrades_core_1.UpgradesError(`Deployment response with id ${deploymentResponse.deploymentId} does not include a transaction hash`, () => 'The Hardhat Upgrades plugin is not currently compatible with this type of deployment. Use a relayer for your default deploy approval process in Defender.');
    }
